...
namespace UniShogi
{
    public class Position
    {
        ...
        public void DoMove(Move m)
        {
            if (!IsLegalMove(m))
            {
                throw new ArgumentException($"{m.ToUsi()} は合法手ではありません、局面：{this}");
            }
        ...
        private void SumUpBitboards(Color c)
        {
            silvers[(int)c] = PieceBB(c, Piece.Silver)
                ...
                | PieceBB(c, Piece.P_Bishop)
                | PieceBB(c, Piece.P_Rook);
            golds[(int)c] = PieceBB(c, Piece.Gold)
                | PieceBB(c, Piece.King)
                | PieceBB(c, Piece.P_Pawn)
                | PieceBB(c, Piece.P_Lance)
                | PieceBB(c, Piece.P_Knight)
                | PieceBB(c, Piece.P_Silver)
                | PieceBB(c, Piece.P_Bishop)
                | PieceBB(c, Piece.P_Rook);
            bishops[(int)c] =
                PieceBB(c, Piece.Bishop) | PieceBB(c, Piece.P_Bishop);
            rooks[(int)c] =
                PieceBB(c, Piece.Rook) | PieceBB(c, Piece.P_Rook);
        }
